...
//! ASMR_TEST_CONTENT=.../vec-768-layer-12.onnx \
//! ASMR_TEST_RMVPE=.../rmvpe.onnx \
//! ASMR_TEST_WAV=/tmp/piner16k.f32 \
//!   cargo test -p asmr-vc probe_shared_models -- --ignored --nocapture
//! ```
...
fn probe_shared_models() {
    let content = std::env::var("ASMR_TEST_CONTENT").expect("ASMR_TEST_CONTENT");
    let rmvpe = std::env::var("ASMR_TEST_RMVPE").expect("ASMR_TEST_RMVPE");
    let wav = std::env::var("ASMR_TEST_WAV").expect("ASMR_TEST_WAV");
