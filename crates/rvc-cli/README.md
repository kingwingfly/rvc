...
This page documents the standalone `rvc` binary. Every subcommand below is also
available from the `voice` binary as `voice rvc …` — same code, same flags — if
you want speech recognition and synthesis alongside it. See the
...
cargo build --release -p rvc-cli   # -> target/release/rvc
```
...
[`crates/rvc-train/ARCHITECTURE.md`](../rvc-train/ARCHITECTURE.md) for the
implementation.
...
- Speech recognition and synthesis — being built in the `voice` binary, not here;
