//! Bake LibTorch's search path into `voice` so running it needs no environment.
//!
//! A deliberate duplicate of `crates/rvc-cli/build.rs`: an rpath is a property
//! of one linked executable, so every binary in the workspace that links
//! LibTorch needs its own copy. Without it a missing `libtorch.so` aborts in
//! `ld.so` before `main`, on every subcommand — including ones that never touch
//! a GPU, like `voice completions`.
//!
