...
/// Resolve the cache directory: `$ASMR_CACHE_DIR`, else `$XDG_CACHE_HOME/asmr`,
/// else `~/.cache/asmr`.
pub fn default_cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("ASMR_CACHE_DIR") {
        return PathBuf::from(dir);
    }
    if let Ok(xdg) = std::env::var("XDG_CACHE_HOME") {
        return PathBuf::from(xdg).join("asmr");
    }
    if let Ok(home) = std::env::var("HOME") {
        return PathBuf::from(home).join(".cache").join("asmr");
    }
    PathBuf::from(".asmr-cache")
}
