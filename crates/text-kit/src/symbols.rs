//! GPT-SoVITS's v2 phoneme vocabulary.
//!
//! Verbatim and in order, because these indices address the T2S model's phoneme
//! embedding: a table that disagrees with the checkpoint by one entry produces
//! confident nonsense rather than an error. Upstream builds it by sorting a union
//! of per-language sets and then *appending* two more groups unsorted, so it is
//! reproduced here as data rather than as that construction.
//!
//! Generated from `GPT_SoVITS/text/symbols2.py`.

use std::collections::HashMap;
use std::sync::OnceLock;

/// Every symbol the v2 models know, in embedding order.
...
/// The symbol emitted for anything outside the table.
...
fn index() -> &'static HashMap<&'static str, usize> {
    ...
    INDEX.get_or_init(|| SYMBOLS.iter().copied().zip(0..).collect())
}

/// Look up one symbol, falling back to [`UNKNOWN`].
pub fn id(symbol: &str) -> usize {
    let index = index();
    index
        .get(symbol)
        .or_else(|| index.get(UNKNOWN))
        .copied()
        .unwrap_or(0)
}

/// Map phonemes to embedding indices, substituting `UNK` for strangers.
pub fn to_ids<S: AsRef<str>>(phones: &[S]) -> Vec<usize> {
    phones.iter().map(|p| id(p.as_ref())).collect()
}
...
mod tests {
    ...
    #[test]
    fn the_table_matches_the_checkpoints_embedding() {
        // 732 is what upstream's `symbols2.py` prints; the model's phoneme
        // embedding is sized from it, so this number is a compatibility claim.
        assert_eq!(SYMBOLS.len(), 732);
        assert_eq!(SYMBOLS[0], "!");
        assert!(SYMBOLS.contains(&UNKNOWN));
        // Duplicates would shift every later id.
        let unique: std::collections::HashSet<_> = SYMBOLS.iter().collect();
        assert_eq!(unique.len(), SYMBOLS.len(), "duplicate symbol");
    }

    #[test]
    fn strangers_become_unk_rather_than_zero() {
        // Silently mapping to 0 would feed the pad symbol into the model, which
        // is a plausible-looking wrong answer instead of an obvious one.
        assert_eq!(id("\u{1f600}"), id(UNKNOWN));
        assert_ne!(id(UNKNOWN), 0);
    }
}
