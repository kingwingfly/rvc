...
| `burn-gptsovits` | the GPT-SoVITS network. `hubert` at 210/0, `quantizer` at 3/0, and `s2`'s `enc_q`+`flow`+`dec` at 517/0 (of 776 — the harness's `unused` count is the progress bar). `enc_p` and `ref_enc` complete `s2`; the T2S transformer follows. `examples/keys` lists any checkpoint's tensors, which is the first thing to run against a new one |
